...
function doGet(e) {
  ...
  if (action == 'get_feed') {
    try {
      ...
      for(var u=1; u<userData.length; u++) {
        // เช็คว่ามี ID ไหม ถ้าไม่มีให้ข้าม
      ...
      // ฟังก์ชันช่วยแปลง ID -> Avatar Object
      var getAvatars = function(list) {
        ...
        return list.map(function(item) {
          try {
            ...
            if (u) {
               // Clone Object เพื่อไม่ให้กระทบตัวแปรหลัก
      ...
      for (var i = actData.length - 1; i >= 1; i--) {
        if (count >= limit) break; // เอาจำนวนตาม limit
        
        // 🛡️ Try-Catch ในลูป: ถ้าแถวไหนพัง ให้ข้ามไปแถวต่อไป ไม่ล่มทั้งระบบ
        try {
          var row = actData[i];
          if (!row[2]) continue; // ถ้าไม่มี User ID คนโพสต์ ให้ข้าม
          ...
          try { 
            if(row[9] && typeof row[9] === 'string' && row[9].trim() !== "") {
               interactions = JSON.parse(row[9]); 
            }
          } catch(e) { /* JSON ผิดพลาด ช่างมัน */ }
          ...
          // อ่าน Privacy (ถ้าไม่มี Column นี้ ให้ถือเป็น public)
          ...
          // ถ้าแถวนี้มีปัญหา ข้ามไปเงียบๆ
          console.log("Error processing row " + i + ": " + innerErr);
        }
      }
      ...
      // ถ้าพังทั้งหมดจริงๆ ส่ง Error กลับไปบอกหน้าบ้าน
      return responseJSON({status: 'error', message: err.toString()}, e.parameter.callback);
    }
  }

  // 1.2 ดึงข้อมูล Users และ Dashboard (คงเดิม)
  ...
  // 1.3 แบบสอบถามรายเดือน (Survey)
  if (action == 'submit_presurvey') {
    var surveySheet = ss.getSheetByName('Surveys');
    if (!surveySheet) {
      surveySheet = ss.insertSheet('Surveys');
      surveySheet.appendRow(['Timestamp', 'UID', 'Q1_HappyLevel', 'Q2_Teamwork', 'Q3_Pride']);
    }
    surveySheet.appendRow([
      new Date(),
      e.parameter.uid || 'Anonymous',
      e.parameter.q1 || '',
      e.parameter.q2 || '',
      e.parameter.q3 || ''
    ]);
  ...
  // 1.4 ดึงประกาศกิจกรรม (Announcements)
  if (action == 'get_announcements') {
    try {
      var annSheet = ss.getSheetByName('Announcements');
      if (!annSheet) {
        // ✅ สร้างชีตอัตโนมัติ + ใส่ข้อมูลตัวอย่างให้กระดิ่งแสดงผลได้ทันที
        console.log("Sheet 'Announcements' not found. Creating it with sample data...");
        annSheet = ss.insertSheet('Announcements');
        annSheet.appendRow(['ID', 'Title', 'Body', 'EventDate', 'Category', 'PostedBy', 'Timestamp']);
        // ใส่ข้อมูลตัวอย่างให้ badge แสดง
        annSheet.appendRow([
          'ann_welcome_001',
          'ยินดีต้อนรับสู่ระบบ ดี มีสุข!',
          'เริ่มบันทึกกิจกรรมความดีและสะสมคะแนนได้เลยครับ 😊',
          new Date(),
          'general',
          'admin',
          new Date()
      ...
      var today = new Date(new Date().toLocaleString("en-US", {timeZone: "Asia/Bangkok"}));
      today.setHours(0, 0, 0, 0);
      var result = [];

      for (var i = 1; i < rows.length; i++) {
        var row = rows[i];
        if (!row[0]) continue; // ข้ามถ้าไม่มี ID
        ...
        if (row[3]) {
          ...
          if (!isNaN(eventDate.getTime())) {
            ...
            // แสดงผลค้างไว้ 7 วันหลังวันกิจกรรม เพื่อให้ Admin ตรวจสอบได้ง่าย
            if (eventDate.getTime() < (today.getTime() - (86400000 * 7))) {
              showThis = false;
            }
          }
        }
      ...
      // ✅ เรียง: บันทึกล่าสุดก่อน (ts desc) ประกาศใหม่สุดอยู่บนสุดในกระดิ่ง
      result.sort(function(a, b) {
        // ใช้ ts (เวลาบันทึก) เรียง descending
        var tA = a.ts ? new Date(a.ts).getTime() : 0;
        var tB = b.ts ? new Date(b.ts).getTime() : 0;
        if (tA !== tB) return tB - tA;  // บันทึกหลัง = อยู่ก่อน
        // fallback: ถ้าไม่มี ts ใช้ eventDate descending
        if (!a.date) return 1;
        if (!b.date) return -1;
...
function doPost(e) {
  try {
    ...
    // --- 4. บันทึกกิจกรรม (Submit Activity) - 🔥 ยุบรวมและแก้ Logic คะแนน ---
if (action == 'submit_activity') {
      var actSheet = ss.getSheetByName('Activities');
      var userSheet = ss.getSheetByName('Users');
      
    // 1. จัดการสื่อ (รับค่า Link ก่อน ถ้ามีไฟล์ค่อยทับ)
      var imageUrl = data.image || ""; // 👈 รับค่า Link ที่ส่งมาจากหน้าบ้านก่อนเลย
      
      if (data.uploadId) {
        // ถ้ามีการอัปโหลดไฟล์ ให้สร้างไฟล์แล้วเอา URL ไฟล์มาแทนที่ Link
    ...
    if (action == 'check_user') {
      ...
      if (!userSheet) return responseJSON({status: 'error', message: 'No Users Sheet'});
      ...
      return responseJSON({
          exists: !!foundUser, 
          user: foundUser,
          config: systemConfig // 👈 แนบไปตรงนี้
      });
    }
...
function calculateRealStats(actData, usersData) {
  ...
  for(var i=1; i<usersData.length; i++) {
    if(usersData[i][5]) userMapForCloseness[usersData[i][5]] = usersData[i][1];
  }
  
  // 2. วนลูปกิจกรรม
  for (var i = 1; i < actData.length; i++) {
    var row = actData[i];
    var uid = row[2]; // ID คนโพสต์
  ...
  Object.keys(userStats).forEach(function(k) {
    ...
    s.avgHappy = Math.round(score10 * 10) / 10; 
...
function runDailyTimeDecay() {
  ...
  // กำหนดเงื่อนไข
  const GRACE_PERIOD_DAYS = 7; // ให้เวลา 7 วันก่อนเริ่มลดคะแนน
  const DECAY_AMOUNT = 1;      // ลดวันละ 1 แต้ม
  const MIN_SCORE_LIMIT = 5;   // คะแนนความสุขลดต่ำสุดได้แค่ 5 (ไม่ให้เป็น 0)
  
  const today = new Date();
  let updatedCount = 0;

  // สมมติว่าโครงสร้างชีต Users คือ:
  // [0] LineID, [1] Name, [2] Role, [3] TotalScore, [4] HappyScore, [5] LastActiveDate
  // อ้างอิงจากรหัสก่อนหน้า: 
  // [3] = TotalScore (คอลัมน์ D), [4] = HappyScore (คอลัมน์ E), [9] = LastRescueDate (คอลัมน์ J) 
  // วันใช้งานล่าสุดอาจจะไม่มี (ต้องยึดจากที่เพิ่มให้ หรือใช้ LastRescue ก็ได้)
  // แต่จากตรรกะเดิม เรายังไม่ได้บันทึก LastActiveDate ลงชีตเลยครับ 
  // เราจะด่วนสรุปเอาจาก Activities ก็ได้ หรือจากคอลัมน์ที่คุณบอกมา (คอลัมน์ F: วันที่ใช้งานล่าสุด หรือ [5])
  
  for (let i = 1; i < data.length; i++) { // เริ่มที่ 1 เพื่อข้าม Header
    let lastActive = new Date(data[i][5]); // อ่านวันที่ใช้งานล่าสุด (คอลัมน์ F)
    let currentHappyScore = parseFloat(data[i][4]); // อ่านคะแนนความสุขปัจจุบัน (คอลัมน์ E)
    
    // ถ้าอ่านวันที่ได้ และมีคะแนนความสุข
    if (!isNaN(lastActive.getTime()) && !isNaN(currentHappyScore)) {
      
      // คำนวณว่าหายไปกี่วันแล้ว
      let diffTime = Math.abs(today - lastActive);
      let diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
      
      // ถ้าหายไปนานกว่า 7 วัน และคะแนนยังมากกว่าขั้นต่ำ
      if (diffDays > GRACE_PERIOD_DAYS && currentHappyScore > MIN_SCORE_LIMIT) {
        
        let newScore = currentHappyScore - DECAY_AMOUNT;
        if (newScore < MIN_SCORE_LIMIT) newScore = MIN_SCORE_LIMIT; // กันไม่ให้ลดเกินขีดจำกัด
        
        // อัปเดตคะแนนกลับลงไปในชีต (แถว i+1, คอลัมน์ที่ 5/E)
  ...
  console.log(`ดำเนินการ Time Decay เสร็จสิ้น: ลดคะแนนผู้ใช้ไปทั้งหมด ${updatedCount} คน`);
}